...
class FSDPWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_jsd_metrics(self, data: DataProto):
        """Compute true token-level JSD and entropy from full vocabulary distributions.

        The input data should contain interleaved (normal, noisy) pairs:
            data[0] = normal_0, data[1] = noisy_0, data[2] = normal_1, ...
        Both normal and noisy share the same response tokens (from normal generation).
        """
        assert self._is_actor
        data = data.to("cuda")
        if self._use_param_offload:
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            jsd_t, h_t, h_noisy_t, kl_t = self.actor.compute_jsd_metrics(data=data)
            tensors_dict = {"jsd_t": jsd_t, "h_t": h_t, "h_noisy_t": h_noisy_t}
            if kl_t is not None:
                tensors_dict["kl_t"] = kl_t
            output = DataProto.from_dict(
