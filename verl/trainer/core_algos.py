...
def compute_policy_loss(
    ...
    eos_mask: torch.Tensor,
    cliprange: float,
    jsd_mask: torch.Tensor = None,
    jsd_mask_mode: str = "mask",
    jsd_mask_lambda: float = 1.0,
    ...
    Args:
        ...
        jsd_mask: `(torch.Tensor)` optional
            shape: (bs, response_length). Binary mask from JSD TopK selection.
            In "mask" mode: effective mask becomes eos_mask * jsd_mask.
            In "weight" mode: w_t = 1 + lambda for selected tokens, 1 otherwise.
        jsd_mask_mode: (str)
            "mask": binary mask, only selected tokens contribute to loss.
            "weight": token-level weighting, w_t = 1 + lambda for TopK, 1 otherwise.
        jsd_mask_lambda: (float)
            lambda for weight mode: w_t = 1 + lambda for selected tokens.
    ...
    per_token_loss = torch.max(pg_losses, pg_losses2)

    if jsd_mask is not None:
        if jsd_mask_mode == "mask":
            # Binary mask: only train on selected tokens
            effective_mask = eos_mask * jsd_mask
            pg_loss = VF.masked_mean(per_token_loss, effective_mask)
        elif jsd_mask_mode == "weight":
            # Token-level weighting: w_t = 1 + lambda for TopK, 1 otherwise
            token_weights = 1.0 + jsd_mask_lambda * jsd_mask  # (bs, resp_len)
            weighted_loss = per_token_loss * token_weights
            pg_loss = VF.masked_mean(weighted_loss, eos_mask)
        else:
            raise ValueError(f"Unknown jsd_mask_mode: {jsd_mask_mode}. Must be 'mask' or 'weight'.")
    else:
        pg_loss = VF.masked_mean(per_token_loss, eos_mask)
