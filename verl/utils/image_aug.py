...
def _create_black_image(image):
    """Create a black image with the same size and mode as the original image."""
    return Image.new(image.mode, image.size, color=(0, 0, 0))


def remove_images_from_batch(batch):
    """Replace all images with black images to simulate a no-image condition.
    
    We use black images (instead of removing images entirely) to keep the
    vision placeholder tokens aligned with multimodal embeddings, avoiding
    ValueError from vLLM's merge_multimodal_embeddings.
    """
    from copy import deepcopy
    new_batch = deepcopy(batch)
    
    # For vLLM rollout: replace PIL images with same-size black images
    if "multi_modal_data" in new_batch.non_tensor_batch:
        for i, item in enumerate(new_batch.non_tensor_batch["multi_modal_data"]):
            if "image" in item:
                new_batch.non_tensor_batch["multi_modal_data"][i]["image"] = [
                    _create_black_image(img) for img in item["image"]
                ]
    
    # For HuggingFace training forward: zero out pixel_values tensor
    if "multi_modal_inputs" in new_batch.non_tensor_batch:
        for i, item in enumerate(new_batch.non_tensor_batch["multi_modal_inputs"]):
            if "pixel_values" in item:
                new_batch.non_tensor_batch["multi_modal_inputs"][i]["pixel_values"] = \
...
def augment_batch(batch, config, step=0, total_steps=1):
    ...
    # Check noisy_image_mode: "no_image" removes images entirely
    noisy_image_mode = getattr(config, 'noisy_image_mode', 'augment')
    if noisy_image_mode == "no_image":
        return remove_images_from_batch(batch)
    
    # Default "augment" mode: apply image augmentation
