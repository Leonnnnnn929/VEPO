...
DEFAULT_LLM_EVAL_BASE_URL = DEFAULT_OPENROUTER_BASE_URL
DEFAULT_LLM_EVAL_MODEL = DEFAULT_OPENROUTER_MODEL
DEFAULT_OPENROUTER_TIMEOUT = 120
DEFAULT_OPENROUTER_MAX_RETRIES = 6
DEFAULT_OPENROUTER_MIN_INTERVAL = 0.5
DEFAULT_OPENROUTER_MAX_CONCURRENCY = 2

_openrouter_rate_lock = threading.Lock()
_openrouter_last_request_time = 0.0
_openrouter_semaphore = threading.BoundedSemaphore(
    value=max(1, int(os.getenv("OPENROUTER_MAX_CONCURRENT_REQUESTS", DEFAULT_OPENROUTER_MAX_CONCURRENCY)))
)
...
def _call_openai_compatible(prompt: str) -> str:
    api_key = (
        os.getenv("LLM_EVAL_API_KEY", "").strip()
        or os.getenv("OPENROUTER_API_KEY", "").strip()
    )
    if not api_key:
        raise ValueError("LLM_EVAL_API_KEY is not set")

    model = (
        os.getenv("LLM_EVAL_MODEL", "").strip()
        or os.getenv("OPENROUTER_MODEL", "").strip()
        or DEFAULT_LLM_EVAL_MODEL
    )
    base_url = (
        os.getenv("LLM_EVAL_BASE_URL", "").strip()
        or os.getenv("OPENROUTER_BASE_URL", "").strip()
        or DEFAULT_LLM_EVAL_BASE_URL
    ).rstrip("/")
    ...
    site_url = (
        os.getenv("LLM_EVAL_HTTP_REFERER", "").strip()
        or os.getenv("OPENROUTER_HTTP_REFERER", "").strip()
    )
    site_name = (
        os.getenv("LLM_EVAL_X_TITLE", "").strip()
        or os.getenv("OPENROUTER_X_TITLE", "").strip()
    )
    if site_url:
    ...
    max_retries = int(os.getenv("OPENROUTER_MAX_RETRIES", DEFAULT_OPENROUTER_MAX_RETRIES))
    min_interval = float(os.getenv("OPENROUTER_MIN_INTERVAL", DEFAULT_OPENROUTER_MIN_INTERVAL))
    timeout = int(os.getenv("OPENROUTER_TIMEOUT", DEFAULT_OPENROUTER_TIMEOUT))
    ...
    with _openrouter_semaphore:
        for attempt in range(max_retries):
            with _openrouter_rate_lock:
                ...
                elapsed = time.monotonic() - _openrouter_last_request_time
                if elapsed < min_interval:
                    time.sleep(min_interval - elapsed)
                _openrouter_last_request_time = time.monotonic()
            ...
            if response.status_code == 429:
                ...
                wait_time = float(retry_after) if retry_after else min(30.0, 2 ** attempt)
                last_error = RuntimeError(
                    f"LLM eval API rate limited the request (429). Retrying in {wait_time:.1f}s."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"LLM eval API request failed with status {response.status_code}: {error_message}"
                ) from exc
            ...
            if error_message:
                last_error = RuntimeError(f"LLM eval API returned an error payload: {error_message}")
            else:
                last_error = RuntimeError(
                    f"LLM eval API returned an unexpected payload without choices: {str(data)[:500]}"
                )
            time.sleep(min(10.0, 2 ** attempt))

    raise last_error or RuntimeError("LLM eval API request failed for an unknown reason")
...
def _call_gemini(prompt: str) -> str:
    ...
    return model.generate_content(prompt, generation_config={"temperature": 0.0}).text.strip()


def judge_with_api(prompt: str) -> str:
    if os.getenv("LLM_EVAL_API_KEY", "").strip() or os.getenv("OPENROUTER_API_KEY", "").strip():
...
def llm_eval_score(question, prediction, answer, dataset):
    if dataset.lower() == "mathverse":
        ...
        if not boxed_flag:
            ...
            extracted_answer = judge_with_api(extract_prompt)

        score_prompt = build_score_prompt(question, extracted_answer, answer)
        
        for _ in range(3):
            response_text = judge_with_api(score_prompt).strip()
        ...
        extracted_answer = judge_with_api(extract_prompt).strip().upper()
